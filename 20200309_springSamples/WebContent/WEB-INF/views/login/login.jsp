						...
						<table class="content_table" style="width:75%">
							...
							<tbody>
								<tr>
									...
									<td>&nbsp;
										<input type="text" id="_userid" name="id" data-msg="ID를 " size="20px" title="아이디"
											style="border: 1px solid #ddd">
										<input type="checkbox" id="chk_save_id" > ID 저장 
									</td>
								</tr>
								<tr>
									<th style="background: #eee; color:#3e5fba">비밀번호</th>
									<td>&nbsp;
										<input type="password" id="_pwd" name="pwd" data-msg="password를 " size="20px" title="패스워드"
											style="border: 1px solid #ddd">
									</td>
								</tr>
								<tr>
...
$("#_btnLogin").click(function(){
	if( $("#_userid").val().trim() == "" ){
		...
		$("#_frmForm").attr({"action":"loginAf.do", "target" : "_self"});
		$("#_frmForm").submit();
	}
...
$("#chk_save_id").click(function(){
	if($("#chk_save_id").is(":checked") ){	// checked 
		if( $("#_userid").val().trim() == "" ) {
			alert('아이디를 입력해주세요 ');
			$("#chk_save_id").prop("checked",false);
		}else{	// id를 쿠키에 넣기 
			$.cookie("user_id",$("#_userid").val().trim(), { expires: 365, path:'/'});	// path: 기본경로로 저장 , expires 
